...
class TodoItems(models.Model):
    task_id = models.SlugField(
        max_length=26, unique=True, default=ulid.new, editable=False)
    task_title = models.CharField(verbose_name='タイトル', max_length=255)
    task_description = models.TextField(
        verbose_name='詳細', default='', null=True, blank=True)
    ...
    priority = models.SmallIntegerField(
        verbose_name='優先度', default=0, choices=Priorities.get_priorities())
    due_date = models.DateField(verbose_name='期限日', null=True, blank=True)
    created_at = models.DateTimeField(verbose_name='作成日', auto_now_add=True)
    updated_at = models.DateTimeField(verbose_name='更新日', auto_now=True)
    completed_at = models.DateTimeField(
        verbose_name='完了日', null=True, blank=True)
